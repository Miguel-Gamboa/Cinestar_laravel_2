...
@section('contenido_interno')
    ...
    <div class="cine-info">
        <div class="cine-info datos">
            <p>{{ $cine->Direccion }} - {{ $cine->Ciudad }} - {{ $cine->Distrito }}</p>
            <p>Nº Salas: {{ $cine->Salas }} - Formatos: {{ $cine->Formatos }}</p>
            <p>Teléfono: {{ $cine->Telefonos }} anexo 865</p>
            <br/>

            <div class="tabla">
                @foreach($cine->tarifas as $index => $tarifa)
                    <div class="fila {{ $index % 2 == 0 ? 'impar' : 'par' }}">
        ...
        <div class="cine-info peliculas">
            <div class="tabla">
                ...
                @foreach($cine->peliculas as $index => $pelicula)
                    <div class="fila {{ $index % 2 == 0 ? 'impar' : 'par' }}">
